...
- Required checks and CodeRabbit must complete before every merge. Pasta-Devs developers may merge to `staging` after those gates; outside and first-time contributors also require a developer approval. Only `SpicyMarinara` may promote `staging` to `main`.
