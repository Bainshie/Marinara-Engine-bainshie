...
- Added Browser Personal Extension API version 5 with read-only active chat and Character identifiers. Extensions such as Notepad can now keep per-chat or per-Character state in their own private storage and receive live context changes, while the sandbox still exposes no messages, cards, Persona data, metadata, DOM, database, network, or mutation access (#4316).
