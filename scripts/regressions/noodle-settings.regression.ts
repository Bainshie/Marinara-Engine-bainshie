...
try {
  ...
  assert.deepEqual(concurrentlyUpdatedAccount?.settings.scheduler, {
    autoPosting: { enabled: false, intensity: 1, imagesEnabled: false, maxImagesPerRun: 1, nextRunAt: null },
  });
  ...
  assert.deepEqual(normalizedLegacyAccount?.settings, {
    ...
    scheduler: { autoPosting: { enabled: false, intensity: 1, imagesEnabled: false, maxImagesPerRun: 1, nextRunAt: null } },
